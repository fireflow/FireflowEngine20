...
public interface CalendarService extends RuntimeContextAware,EngineModule{
    ...
    /**
     * 返回某一天的工作时间。<br/>
     * 例如8:30-12:00 & 13:30-17:30 表示早上8点半到中午12点和下午1点半到5点半<br/>
     * 例如9:00-12:00 表示这一天只有上午半天班。<br/>
     * 例如8:30-12:00 & 13:30-17:30 & 19:30-22:00， 表示这一天除了上午、下午之外还有晚班，晚班从19:30-22:00<br/><br/>
     * 
    ...
    /**
     * 
     * 每天的工作时长，该返回值主要用于函数{@link #dateAfter(Date, Duration) dateAfter(Date, Duration)}的计算。<br/><br/>
     * 
     * 例如：如果{@link #getBusinessTime getBusinessTime(argDate)}的返回值是"8:30-12:00 & 13:30-17:30"，则每天的工作时间长度是7个半小时，
     * 那么本方法的返回值应该是7.5。因此本方法的返回值要和{@link #getBusinessTime(argDate) getBusinessTime(argDate)}要相匹配。<br/><br/>
     * 
     * 如果每天工作时间不相同（例如周一至周五上全天班，周六上半天班），且对于流程超时设置不严格的情况下，可以返回-1。如果返回-1，
     ...
     * 示例1：如果fromDate是2013-01-01 00:00:00，<strong>4天</strong>后的日期时间是2013-01-05 00:00:00。<br/>
     * 此时参数duration的值如下:duration.value=4,duration.unit=DAY,duration.isBusinessTime=false;<br/><br/>
     * 
     * 示例2：如果fromDate是2013-01-01 00:00:00， <strong>4个工作日</strong>后的日期时间是2013-01-08 00:00:00。
     * (假设周六周日是休息日，周一至周五是工作日)<br/>
     * 此时参数duration的值如下:duration.value=4,duration.unit=DAY,<strong>duration.isBusinessTime=true;</strong><br/><br/>
     * 
     * 在缺省实现缺省实现 {@link org.fireflow.engine.modules.calendar.impl.CalendarServiceDefaultImpl CalendarServiceDefaultImpl}中只区分工作日/自然日，工作时/自然时；
