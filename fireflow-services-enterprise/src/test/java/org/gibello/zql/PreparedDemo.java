...
public class PreparedDemo {
  ...
  public static void main(String args[]) {
    try {
      InputStream in = PreparedDemo.class.getResourceAsStream("PreparedDemo.txt");
      ZqlParser p = null;

      p = new ZqlParser(new DataInputStream(in));
//      if(args.length < 1) {
...
//      }
  ...
  static void handleUpdate(ZUpdate upd) throws Exception {
    ...
    int count = upd.getColumnUpdateCount();
    String tab = upd.getTable();
    Hashtable set = upd.getSet();
    
    for (int i=1;i<=count;i++){
    	String name = upd.getColumnUpdateName(i);
