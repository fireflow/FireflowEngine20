 ...
 *
 */
public class WorkflowStatementAbortActivityInstanceTest  extends FireWorkflowJunitEnviroment {
	...
	public void testAbortProcessInstance() {
		...
		query2.add(Restrictions.eq(ActivityInstanceProperty.PROCESS_INSTANCE_ID, processInstanceId))
				.add(Restrictions.eq(ActivityInstanceProperty.NAME, "Activity2"));
		final ActivityInstance actInst = query2.unique();

		transactionTemplate.execute(new TransactionCallback(){
			public Object doInTransaction(TransactionStatus arg0) {
				try {
					stmt.abortActivityInstance(actInst.getId(), note);
				} catch (InvalidOperationException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}catch(Exception e){
	...
	public void assertResult(WorkflowSession session){
		...
		Assert.assertEquals(ActivityInstanceState.ABORTED, actInst.getState());
